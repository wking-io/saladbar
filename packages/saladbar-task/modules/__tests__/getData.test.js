...
test('getData returns value of data attribute on task element', assert => {
  const document = createElement(1, {
    attrs: ['data-test="true"'],
    classes: 'default',
  });
  const taskEl = Task.of(document.querySelector('.default'));
  const actual = getData('test', taskEl);
  const expected = 'true';
  actual.fork(
    () => assert.fail('getData returned an error.'),
    attr => assert.equal(attr, expected)
...
test('getData returns error if data attribute not found on task element', assert => {
  const document = createElement(1, {
    attrs: ['data-test="true"'],
    classes: 'default',
  });
  const taskEl = Task.of(document.querySelector('.default'));
  const actual = getData('not-real', taskEl);
  const expected = true;
  actual.fork(
    err => assert.equal(err.hasOwnProperty('error'), expected),
    () => assert.fail('getData did not return an error.')
